...
import { Box, Button, PhoneInput, Text } from '@/src/components/ui';
...
const LoginCard: React.FC<CardProps> = ({ phone, setPhone, handleSendOtp }) => {
  const [isLoading] = useState(false); // TODO: Replace with actual loading state from API
  ...
  return (
    ...
    >
      <LogoSvgComponent style={{ alignSelf: 'center', marginBottom: 24 }} />
      <Text variant={'header'} marginBottom={'m'} textAlign={'center'}>
        {t("login.screenTitle") || "Welcome"}
      </Text>
      <Text
        variant={'subheader'}
        textAlign={'center'}
      ...
      >
        {t("login.screenDescription") || "Enter your phone number to continue"}
      </Text>
      <PhoneInput
        value={phone}
      ...
      />
      <Button
        label={t("login.buttonTitle") || "Send OTP"}
